extensions [ web props ] ;custom-logging]
...
[
  ...
  sensor-placed                     ;;currently dragged sensor to avoid placing > 1 sensors per click
...
breed [ sensor-labels sensor-label ]

erasers-own [pressure?]
flashes-own [birthday]
sensors-own [ my-call number reading my-history my-running-history light-up-threshold index ]
sensor-labels-own [ my-sensor showlabel ]
...
to setup
  ca 
  set root-url-for-export "http://abmplus.tech.northwestern.edu:9001/submit"
  ...
  set sensor-colors [  gray orange violet lime  ]
  set sensor-placed nobody
...
to sensor-action
  ask sensors [
   let the-colors [ white orange violet green ]
   let colornames [ "none" "orange" "purple" "green" ]
   let sense-radius 2
   
   let mytype   item number colornames
   let rulename (word mytype "-sensor-rule")
   let ruleval run-result rulename
   let the-color item number the-colors ; colornames
   let delta  count particles with [ color-type = the-color ] in-radius sense-radius   ;;HERE IS THE LINE TO MODIFY IF YOU WANTED TO ALTER SENSOR BEHAVIOR TO SENSE MORE COLORS.
   add-to-records delta
   let oreading reading
   carefully 
   [ 
     ifelse member? "now" ruleval [ 
       set reading delta
     ]
     [
       ifelse member? "last" ruleval [
         set reading round ( sum my-running-history )
       ]
       [
         set reading round ( sum my-history )
       ] 
     ]
   ]
   [ user-message (word "oops:  " error-message "  in running sensor code" ) ]
...
to add-to-records [ avalue ]
  let tvalue  (avalue * tick-delta * 2)
  record tvalue
  running-record tvalue
end
...
to running-record [ avalue ]
  set my-running-history lput avalue my-running-history
  let desiredlen round (30 / tick-delta)
  let len length my-running-history
  if (length my-running-history > desiredlen ) [ set my-running-history sublist my-running-history (len - desiredlen) (len - 1) ]
end
  ...
  ifelse mouse-down? [
   ; every .1 [ custom-logging:log-message (word mouse-interaction " at: " snap-xcor ", " snap-ycor "; pause-particles? = " pause-particles?) ]
    ask patches with [pxcor = snap-xcor and pycor = snap-ycor] [
      ...
      if member? "particle sensor" mouse-interaction 
      [
        
        let existing-ones sensors with [my-call = mouse-interaction]
        ifelse (count existing-ones < 4 and sensor-placed = nobody ) 
        [
          ifelse (any? existing-ones in-radius 2)
          [
            set sensor-placed one-of existing-ones in-radius 2
          ]
          [
            sprout-sensors 1 
            [
              ...
              if (member? "orange" mouse-interaction) [set number 1 ]
              if (member? "purple" mouse-interaction) [ set number 2 ]
              if (member? "green" mouse-interaction) [ set number 3 ]
              set color item number sensor-colors
              set sensor-placed self
              hatch-sensor-labels 1 
              [
                set my-sensor myself
                setxy xcor + 2.2 ycor + 1.4
                set size .1
                set label-color color + 2
                set showlabel (word "index " [index] of myself)
      ...
      if mouse-interaction = "delete sensors" [
       if any? sensors in-radius 2 [ 
         ask min-one-of sensors [ distance myself ] 
         [ 
           ask sensor-labels with [ my-sensor = myself ] [ die ]
           die 
         ] 
        ] 
      ]
...
;;note - root-url-for-export  global variable - http://abmplus.tech.northwestern.edu:9001/submit
;;for local export, need global variable last-saved-filename


to publish [modifier]
  let period_id_prop "testperiod"   ;default value for offline
  let user_id_prop   "testuser"     ;default value for offline
  let run_id_prop    "testrun"      ;default value for offline
  let is_original_prop true
  let model_url_prop  ""
  
  if (props:get "jnlp.netlogo.period_id" != false) 
  [
    set period_id_prop props:get "jnlp.netlogo.period_id"
    let props-user props:get "jnlp.netlogo.user_id"
    ifelse (member? "-final" props-user)
    [
      set user_id_prop   props-user  ;once it's final work, it always is traced with that classification.
    ]
    [
      set user_id_prop   (word props-user modifier)  ;otherwise, you can append -final or not, depending on user choice
    ]
    set run_id_prop    props:get "jnlp.netlogo.run_id"
    set is_original_prop props:get "jnlp.netlogo.is_original" 
    set model_url_prop props:get "jnlp.netlogo.model_source_url"
  ]
  
  let my-description  user-input "Provide a brief description for the Sandbox setup that you have created..."
  
  if (my-description = false ) [ stop ]
  
  ;;no turning back now, turn the pause-particles switch on
  set pause-particles? true
 ; custom-logging:log-message (word "publish by WISE user: " (props:get "jnlp.netlogo.user_id") ".  Message= " my-description )
  
  let period-id-tuple (list "period_id" period_id_prop)
  let user-id-tuple   (list "user_id"   user_id_prop)
  let run-id-tuple    (list "run_id"    run_id_prop)
  let work-meta-tuple  (list "metadata" (word "{ \"type\": \"export_world\", \"model_url\": \"" model_url_prop "\" }"))
  
  
  let original-world is_original_prop
  if (original-world = false)
   [
    let commenter-name user-input (word "Your Username: (if not " user_id_prop ")" )
    if (commenter-name != false and length commenter-name > 0) 
    [ 
      set my-description (word my-description " (Modification by " commenter-name ")")
    ]
  ]
  
  let work-desc-tuple  (list "description"   my-description)
  
  let work-url      (word root-url-for-export "-work")
  ...
  [
   ...
   ; custom-logging:log-message (word "Gallery work ID: " work-response "; User" (props:get "jnlp.netlogo.user_id") ".  Message= " my-description )
    ifelse (member? "Entity Too Large" item 1 work-response) 
    [ 
      user-message "<HTML><H1>Your world-state file is too large to send to the server</H1><H2>You can still save it locally, however.</H2></HTML>"
    ]
    [
      ...
      let extras-url      (word root-url-for-export "-supplement")
...
to save-locally
  ...
  ;show fname
  ifelse (check-filename fname )
  [
    set last-saved-filename (word fname ".csv")
    set pause-particles? true
    export-world (word fname ".csv")
  ]
  [
    user-message (word "You have not entered a valid filename.  Please try again.") 
  ]
end

to overwrite-last-saved
  carefully
  [ 
    if last-saved-filename = "" [ show __boom  ]
    if not file-exists? last-saved-filename [ show __boom  ]
    export-world last-saved-filename ]
  [ user-message "There was an error in saving.\nYou have not saved yet \n(or you have pressed setup since saving) \nTry using the 'save locally' button." ]
end

to-report check-filename [afilename ]
  if afilename = false [ report false ]
  if length afilename < 1 [ report false ]
  report true
end
...
to load-last-saved
  ...
  [ 
    if last-saved-filename = "" [ show __boom  ]
    if not file-exists? last-saved-filename [ show __boom  ]
    import-world last-saved-filename 
    setup-default-shapes
    calculate-tick-delta
  ]
  [ user-message "You have not saved yet\n(or you have pressed setup since saving)\nTry using the 'load another' button." ]
end

to load-another
  let maybe-load user-file
  if (maybe-load != false) [ import-world maybe-load ]
  setup-default-shapes
  calculate-tick-delta
end

to-report check [ thing ]
  ;check for stuff like: export_world/61
  if (length thing < 13) [  report false ]
  if (not member? "export_world/" thing) [ report false ]
  report true
end

to-report prep [ thing ]
  let newthing thing
  
  ;strip trailing spaces
  while [ last newthing = " "] 
  [ set newthing butlast newthing ]
  
  ;strip leading spaces
  while [ first newthing = " "] 
  [ set newthing butfirst newthing ]
  
  report newthing
end

to import-from-url
  let the-thing user-input "Copy-paste a WISE Gallery ID (in the form 'export_world/#')\nto load the experiment state..." 
  set the-thing prep the-thing
  ifelse (check the-thing) 
  [
    let imgonna (word "http://abmplus.tech.northwestern.edu:9001/assets/uploads/" the-thing ".csv.gz")
    ;;show imgonna 
    carefully  
    [web:import-world imgonna  setup-default-shapes calculate-tick-delta] 
    [ user-message (word "Your input, '" the-thing "', did not correspond to any saved state that could be loaded.\n\nPlease try again.") ]
  ]
  [
    user-message (word "Your input, " the-thing ", is not in the correct format\nfor an export_world state identifier.\n\nCheck the Gallery again, and make sure you are using\nthe ID, which will have the form 'export_world/#'." )
  ]
end
...
280
680
Publish to WISE as a Work-In-Progress
publish \"\"
NIL
1
T
...
280
643
SAVE/LOAD/PUBLISH EXPERIMENT SETUP:
13
12.0
1
...
orange-sensor-rule
orange-sensor-rule
"# detected now" "# detected in last 30 seconds" "Total # detected"
2
...
175
578
Show Sensor Index Numbers
ask sensor-labels [ set label showlabel ]
NIL
1
T
OBSERVER
...
175
613
Hide Sensor Index Numbers
ask sensor-labels [ set label \"\" ]
NIL
1
T
OBSERVER
...
280
813
Publish to WISE Poster Session Gallery
publish \"-final\"
NIL
1
T
